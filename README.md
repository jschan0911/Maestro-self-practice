...
- 초대 링크 외에 팀을 추가할 수 있는 다른 방법이 있을까요?
- 합주 중 팀원의 출입이나 다른 조건들(bpm, 일시정지 등)을 실시간으로 변경할 수 있도록 처리할 것인가요?
- 합주했던 기록을 콘티(폴더)나 팀 페이지에 따로 추가하는 것은 어떤가요? 
- 합주를 시작할 수 있는 권한은 어떻게 부여할까요? 여러 사람에게 권한이 있을 경우, 한 팀 내에 여러 개의 합주가 가능할까요?
